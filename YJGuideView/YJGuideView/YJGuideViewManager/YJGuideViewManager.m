//
//  YJGuideViewManager.m
//  YJGuideView
//
//  Created by YJHou on 2016/12/18.
//  Copyright © 2016年 YJHou. All rights reserved.
//

#import "YJGuideViewManager.h"

@implementation YJGuideViewManager
...
-(UIView *)presentModalMessage:(NSString*)message where:(UIView*)presentationPlace{
    ...
    if ([self.hintDelegate respondsToSelector:@selector(hintStateViewsToHint:)]) {
        ...
        if(viewArray!=nil)
            _modalView = [[YJGuideView alloc] initWithFrame:presentationPlace.frame forViews:viewArray];
    }
    ...
    if ([self.hintDelegate respondsToSelector:@selector(hintStateRectsToHint:)]) {
        ...
        if (rectArray != nil)
            _modalView = [[YJGuideView alloc] initWithFrame:presentationPlace.frame withRects:rectArray];
    }
    
    if (_modalView==nil)
        _modalView = [[YJGuideView alloc] initWithFrame:presentationPlace.frame];
